...
namespace Ders18Koleksiyonlar
{
    internal class Program
    {
        ...
        static void Ornek6()
        {
            Stack stack = new Stack(); // Stac sınıfı programlama LIFO - son giren ilk çıkar kuralıyla çalışır.
            // stack.Add(); stac de listeye add metoduyla eklme yapılmaz
            stack.Push("Çankırı");
            stack.Push("Ankara");
            stack.Push("İstanbul");
            stack.Push("Karabük");
            ...
            while (stack.Count > 0)
            {
                Console.WriteLine(stack.Pop()); // stac de nesneler üst üste koyulmuş gibidir verilerede üstten çekerek ulaşılır.
        ...
        static void Ornek8()
        {
            Dictionary<string, string> Dictionary = new(); // hangi veri tipinden oluşacağını belirleyebildiğimiz liste 
            // sistemi
        ...
        static void ListKullanimi()
        {
            ...
            Console.WriteLine();
            kullanicilar.AddRange(users); // kullanıcılar listesine çoklu ekleme için AddRange mwtodu kullanılır.
            Console.WriteLine("Kullanıcılar(kullanicilar) : ");
            foreach (var item in kullanicilar)
            {
                Console.WriteLine(item.Name);
